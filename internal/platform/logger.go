...
func (l *Logger) logLine(prefix, color, format string, args ...any) { // nosemgrep: domain-primitives.multiple-string-params-go -- prefix/color/format are semantically distinct logger internals; ANSI color and format string have fundamentally different uses [permanent]
...
func (l *Logger) Banner(dir domain.BannerDirection, kind, name, description string) { // nosemgrep: domain-primitives.multiple-string-params-go -- kind/name/description are D-Mail display fields used together for banner formatting; semantically distinct [permanent]
	desc := description
	if len(desc) > 50 {
		desc = desc[:47] + "..."
	}
