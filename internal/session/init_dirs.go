...
func relOrBase(stateDir, path, base string) string { // nosemgrep: domain-primitives.multiple-string-params-go -- stateDir/path/base are semantically distinct path resolution params; newtype wrapping adds no safety benefit [permanent]
	if path == stateDir {
		return base + "/"
	}
