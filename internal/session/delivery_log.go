...
func (l *DeliveryLog) Delivered(kind, from, to string) { // nosemgrep: domain-primitives.multiple-string-params-go -- kind/from/to are semantically distinct delivery log fields (DMailKind, outbox path, inbox path) [permanent]
...
func (l *DeliveryLog) Failed(kind, from, reason string) { // nosemgrep: domain-primitives.multiple-string-params-go -- kind/from/reason are semantically distinct delivery log fields (DMailKind, outbox path, error reason) [permanent]
	l.write("FAILED", fmt.Sprintf("kind=%s from=%s reason=%s", kind, from, reason))
}

// Retried records a successful retry delivery.
func (l *DeliveryLog) Retried(kind, from, to string) { // nosemgrep: domain-primitives.multiple-string-params-go -- kind/from/to are semantically distinct delivery log fields (DMailKind, outbox path, inbox path) [permanent]
