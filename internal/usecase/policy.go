...
type PolicyHandler func(ctx context.Context, event phonewave.Event) error
...
type PolicyEngine struct {
	handlers map[phonewave.EventType][]PolicyHandler
...
func NewPolicyEngine(logger *phonewave.Logger) *PolicyEngine {
	return &PolicyEngine{
		handlers: make(map[phonewave.EventType][]PolicyHandler),
		logger:   logger,
	}
}

// Register adds a handler for the given event type.
func (e *PolicyEngine) Register(trigger phonewave.EventType, handler PolicyHandler) {
...
func (e *PolicyEngine) Dispatch(ctx context.Context, event phonewave.Event) error {
